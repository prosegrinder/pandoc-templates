...
$ShunnLongStoryDir=Join-Path $PSScriptRoot "..\shunn\long"
Get-ChildItem $ShunnLongStoryDir

# Create a temporary data directory
$env:PANDOC_DATA_DIR=New-TemporaryDirectory

# Prep the template and reference directories
Copy-Item -Path $ShunnLongStoryDir\template.docx -Destination $env:PANDOC_DATA_DIR\template.zip
...
pandoc $infile --from markdown --to docx --lua-filter $ShunnLongStoryDir/shunnlong.lua --data-dir $env:PANDOC_DATA_DIR --output $outfile
